...
def nome_campeao(dados):
    n = dados["fases"]["2700"]["classificacao"]["grupo"]["Único"][0]
    return dados["equipes"][n]["nome-comum"]
...
def qtos_libertadores(dados):
    n = dados["fases"]["2700"]["faixas-classificacao"]["classifica1"]["faixa"]
    m =  n.split('-')
    return int(m[1])
...
def ids_dos_melhor_classificados(dados,numero_de_times):
    lista = []
    for x in range (numero_de_times):
...
def nomes_classificados_libertadores(dados):
    ...
    for x in range(3):
        lista.append(dados["fases"]["2700"]["classificacao"]["grupo"]["Único"][x])
    for y in lista:
        lista1.append(dados["equipes"][y]["nome-comum"])
    return lista1
...
'''
def ids_dos_times_de_um_jogo(dados,id_jogo):
    a = dados["fases"]["2700"]["jogos"]["id"][id_jogo]["time1"]
    b = dados["fases"]["2700"]["jogos"]["id"][id_jogo]["time2"]
    return a,b
...
def nomes_dos_times_de_um_jogo(dados,id_jogo):
   a = dados["fases"]["2700"]["jogos"]["id"][id_jogo]["time1"]
   b = dados["fases"]["2700"]["jogos"]["id"][id_jogo]["time2"]
   c = dados["equipes"][a]["nome-comum"]
   d = dados["equipes"][b]["nome-comum"]
   return c,d
